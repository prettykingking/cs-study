...
class SortedTableMap(MapBase):
    ...
    def __getitem__(self, k):
        """Return value associated with k (raise KeyError if not found)."""
    ...
    def find_ge(self, k):
        """Return (key, value) pair with least key greater or equal to k."""
    ...
    def find_lt(self, k):
        ...
        j = self._find_index(k, 0, len(self._table) - 1)                # j's key >= k
        if j > 0:
            return (self._table[j-1]._key, self._table[j-1]._value)     # Note of use j-1
        else:
            return None
    ...
    def find_range(self, start, stop):
        ...
        while j < len(self._table) and (stop is None or self._table[j] < stop):
