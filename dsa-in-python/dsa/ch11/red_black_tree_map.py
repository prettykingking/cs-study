...
class RedBlackTreeMap(TreeMap):
    ...
    def _get_red_child(self, p):
        """Get a red child of p (or None if no such child)."""
        ...
        if self.is_root(p):
            ...
            if self._is_red(parent):            # double red problem
                ...
                if not self._is_red(uncle):             # Case 1: misshapen 4 nodes
                    middle = self._restructure(p)       # do trinode restructure
                    self._set_black(middle)             # and then fix colors
                    self._set_red(self.left(middle))
                    self._set_red(self.right(middle))
                else:                                   # Case 2: overfull 5 nodes
    ...
    def _fix_deficit(self, z, y):
        """Resolve black deficit at z, where y is the z's heavier subtree."""
